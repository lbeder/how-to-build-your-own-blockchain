...
const lightningNetwork = new LightningNetwork(blockchain);
...
app.post("/lightning/channel", (req: express.Request, res: express.Response) => {
  const multiAddress = req.body.multiAddress;
  const sendingAddress = req.body.sendingAddress;
  const fundingTxValue = Number(req.body.fundingTxValue);

  if (!multiAddress || !sendingAddress || !fundingTxValue)  {
    res.json("Invalid parameters!");
    res.status(500);
    return;
  }
  
  const bdChannel = lightningNetwork.openCandidateChannel(multiAddress, nodeId, sendingAddress, fundingTxValue);
  if (!bdChannel.isSigned) {
    res.json(`A candidate bi-directional channel was created by node ${nodeId}`);
  }
  else{
    let funding = bdChannel.getFunding();
    res.json(`A bi-directional channel was succesffuly created and joined the lightning-network. Address = ${multiAddress}, Funding = ${funding}`);
  }
  res.status(200);
});

app.post("/lightning/submitTransaction", (req: express.Request, res: express.Response) => {
  const multiAddress = req.body.multiAddress;
  const senderAddress = req.body.senderAddress;
  const destAddress = req.body.destAddress;
  const transactionValue = Number(req.body.transactionValue);

  if (!multiAddress || !senderAddress || !destAddress || !transactionValue)  {
    res.json("Invalid parameters!");
    res.status(500);
    return;
  }
  
  const transaction = lightningNetwork.submitLightningTransaction(nodeId, multiAddress, senderAddress, destAddress, transactionValue);

  res.json(serialize(transaction));
  res.status(200);
});

app.post("/lightning/broadcast", (req: express.Request, res: express.Response) => {
  const multiAddress = req.body.multiAddress;
  ...
  lightningNetwork.broadcastChannelTransaction(nodeId, multiAddress);

  res.json(`Lightning transaction from channel address ${multiAddress} was broadcasted successfully to the blockchain by nodeId ${nodeId}`);
  res.status(200);
});
