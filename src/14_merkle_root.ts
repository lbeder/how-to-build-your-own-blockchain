 ...
 */
export function blockMerkleRoot(txs: Transaction[]): string {
  ...
  function merkleRoot(hashes: string[]): string {
    ...
    for (let i = 0; i < hashes.length; i += 2) {
      nextHashes.push(sha256(hashes[i].concat(hashes[i + 1])));
    }

    return merkleRoot(nextHashes);
  }

  const hashes = txs.map(tx => sha256(JSON.stringify(serialize<Transaction>(tx))));
...
export class BlockHeader {
  ...
  constructor(blockNumber: number, timestamp: number, nonce: number,
    prevBlock: string, merkleRoot: string) {
  ...
  // Calculates the SHA256 of the block header
  public sha256(): string {
    return sha256(JSON.stringify(serialize<BlockHeader>(this)));
  }
}
...
export class Block {
  ...
  constructor(header: BlockHeader, transactions: Array<Transaction>) {
    this.header = header;
...
export class Blockchain {
  // Let's define that our "genesis" block as an empty block, starting from the January 1, 1970 (midnight "UTC") with a single transaction
  private static readonly GENESIS_TRANSACTIONS = [new Transaction("Satoshi Nakamoto", "Satoshi Nakamoto", 1000)];
  public static readonly GENESIS_BLOCK_HEADER = new BlockHeader(0, 0, 0, "fiat lux", blockMerkleRoot(Blockchain.GENESIS_TRANSACTIONS));

  public static readonly GENESIS_BLOCK = new Block(Blockchain.GENESIS_BLOCK_HEADER, Blockchain.GENESIS_TRANSACTIONS);
  ...
  public static verify(blocks: Array<Block>): boolean {
    try {
      ...
      for (let i = 1; i < blocks.length; ++i) {
        ...
        // Verify block number.
        if (current.header.blockNumber !== i) {
          throw new Error(`Invalid block number ${current.header.blockNumber} for block #${i}!`);
        }

        // Verify that the current blocks properly points to the previous block.
        const previous = blocks[i - 1];
        
        if (current.header.prevBlock !== previous.header.sha256()) {
          throw new Error(`Invalid previous block hash for block #${i}!`);
        }

        // Verify merkle root of the transactions
        if (blockMerkleRoot(current.transactions) !== (current.header.merkleRoot)) {
          throw new Error(`Invalid block merkle root ${current.header.merkleRoot} for block #${i}!`);
        }
        
        // Verify the difficulty of the PoW.
        //
        // TODO: what if the difficulty was adjusted?
        if (!this.isPoWValid(current.header.sha256())) {
          throw new Error(`Invalid previous block hash's difficulty for block #${i}!`);
        }
      }
  ...
  private mineBlock(transactions: Array<Transaction>): Block {
    ...
    const header = new BlockHeader(lastBlock.header.blockNumber + 1, Blockchain.now(), 0, lastBlock.header.sha256(), blockMerkleRoot(transactions));

    while (true) {
      const pow = header.sha256();
      console.log(`Mining #${header.blockNumber}: nonce: ${header.nonce}, pow: ${pow}`);
...
app.post("/blocks/mine", (req: express.Request, res: express.Response) => {
  ...
  res.json(`Mined new block #${newBlock.header.blockNumber}`);
});
